...
from tensorflow.contrib.layers import xavier_initializer  #虽然这里有红线, 但是可以直接跑
import numpy as np
...
def defineRandomNameParam(shape, dtype=tf.float32, reg=False, initializer='xavier', trainable=True):
	"""
	作用: 初始化Q, K, V要用的参数矩阵(在 degenerate 中 83行被调用 sequenceModeling)
	:param shape:
	:param dtype:
	:param reg:
	:param initializer:
	...
	:return:
	"""
	name = 'defaultParamName%d'%getParamId()  #自加1函数
...
def defineParam(name, shape, dtype=tf.float32, reg=False, initializer='xavier', trainable=True):
	...
	if initializer == 'xavier':
		ret = tf.get_variable(name=name, dtype=dtype, shape=shape,
			# 会检查当前命名空间下是否存在同样name的变量, 可以方便共享变量。而tf.Variable每次都会新建一个变量。
			initializer=xavier_initializer(dtype=tf.float32),  #初始化对结果的影响还是很大
